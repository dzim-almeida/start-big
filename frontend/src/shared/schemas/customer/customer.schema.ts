...
import { GenderTypeEnum } from './enums/genderEnum.schema';
...
const CustomerBaseSchema = z.object({
  ...
  observacoes: z
    .string()
    .max(500, 'A observação tem uma máximo de 500 caracteres')
    .optional()
    .nullable(),
  endereco: z.preprocess((val) => {
...
export type CustomerPFReadSchemaDataType = z.infer<typeof CustomerPFReadSchema>;
export type CustomerPJReadSchemaDataType = z.infer<typeof CustomerPJReadSchema>;

export const CustomerUnionReadSchema = z.discriminatedUnion('tipo', [CustomerPFReadSchema, CustomerPJReadSchema]);
...
export const CustomerUnionReadPaginatedSchema = z.object({
  ...PaginationBaseSchema.shape,
  filters: CustomerFilterSchema,
  items: z.array(CustomerUnionReadSchema),
});

export type CustomerUnionReadPaginatedSchemaDataType = z.infer<typeof CustomerUnionReadPaginatedSchema>;
export type CustomerUnionReadSchemaDataType = z.infer<typeof CustomerUnionReadSchema>;

export function isCustomerPF(customer: CustomerUnionReadSchemaDataType): customer is z.infer<typeof CustomerPFReadSchema> {
  return customer.tipo === 'PF';
}