...
import type { ModalMode } from '../types/servicos.types';
import type { ServiceReadZod } from '../schemas/servicos.schema';

const isOpen = ref(false);
const mode = ref<ModalMode>('create');
const selectedServico = ref<ServiceReadZod | null>(null);
...
export function useServicoModal() {
  ...
  function openEditModal(servico: ServiceReadZod) {
    selectedServico.value = servico;
    mode.value = 'edit';
    isOpen.value = true;
  }

  function openViewModal(servico: ServiceReadZod) {
    selectedServico.value = servico;
    mode.value = 'view';
    isOpen.value = true;
  }
