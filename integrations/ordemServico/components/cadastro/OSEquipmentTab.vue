...
/**
 * OSEquipmentTab.vue
 * Aba de dados do equipamento na OS
 */
import { Smartphone } from 'lucide-vue-next';

import BaseInput from '@/shared/components/commons/BaseInput/BaseInput.vue';
import type { EquipamentoHistorico } from '@/modules/clientes/types/clientes.types';
...
<template>
  <div class="space-y-5 animate-fadeIn">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <!-- Coluna Esquerda: Dados do Aparelho -->
      <div class="space-y-4">
        <h5 class="flex items-center justify-between text-xs font-bold text-slate-500 uppercase border-b border-slate-100 pb-2">
          ...
          <div v-if="equipamentosHistorico.length > 0 && !isLocked" class="flex items-center gap-2">
            ...
            >
              ...
              <option
                v-for="(equip, idx) in equipamentosHistorico"
                :key="idx"
                :value="idx"
              >
      ...
      <!-- Coluna Direita: Detalhes e Seguranca -->
      <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          <BaseInput
            :model-value="modelValue.imei"
            label="IMEI"
            placeholder="IMEI"
            @update:model-value="updateField('imei', $event)"
          />
          <BaseInput
            :model-value="modelValue.cor"
            label="Cor"
            placeholder="Ex: Preto"
            @update:model-value="updateField('cor', $event)"
          />
        </div>

        <BaseInput
          :model-value="modelValue.senha_aparelho"
          label="Senha / Padrao"
          placeholder="Senha de desbloqueio"
          @update:model-value="updateField('senha_aparelho', $event)"
        />

        <BaseInput
          :model-value="modelValue.acessorios"
          label="Acessorios (Deixados)"
          placeholder="Ex: Carregador, Capa..."
          @update:model-value="updateField('acessorios', $event)"
        />
      </div>
    </div>
    ...
    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
      <div>
        <label class="block text-xs font-bold text-slate-700 mb-1 uppercase">
          Defeito Relatado <span class="text-red-500">*</span>
        </label>
        <textarea
