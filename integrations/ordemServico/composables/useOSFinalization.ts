/**
 * ===========================================================================
 * ARQUIVO: useOSFinalization.ts
 * MODULO: Ordem de Servico
 * DESCRICAO: Composable para gerenciar o fluxo completo de finalizacao de OS.
 *            Inclui solucao, garantia, descontos e registro de pagamentos.
 * ===========================================================================
 *
 * FUNCIONALIDADES:
 * - Formulario de finalizacao (solucao, observacoes, garantia, desconto)
 * - Validacao com Zod (solucao obrigatoria min 3 caracteres)
 * - Gerenciamento de multiplos pagamentos
 * - Busca de formas de pagamento disponiveis
 * - Calculo automatico de subtotal, total, restante e troco
 * - Confirmacao obrigatoria antes de finalizar
 *
 * FLUXO:
 * 1. Preencher solucao aplicada (obrigatorio)
 * 2. Definir dias de garantia
 * 3. Aplicar desconto (opcional)
 * 4. Registrar pagamento(s) ate cobrir o total
 * 5. Confirmar e finalizar
 *
 * RETORNO:
 * - form, errors: Estado do formulario
 * - formasPagamento: Lista de formas disponiveis
 * - addedPayments: Pagamentos registrados
 * - valorTotalFinal, totalPago, restante, troco: Calculos financeiros
 * - canSubmit: Validacao completa do formulario
 * - handleSubmit: Executa finalizacao
 * ===========================================================================
 */
...
export function useOSFinalization(props: UseOSFinalizationProps, emit: UseOSFinalizationEmits) {
  ...
  const { data: formasPagamento } = useQuery({
    ...
    queryFn: () => financeiroService.getFormasPagamento(),
  ...
  const canSubmit = computed(() => {
    // Validate schema implicitly for button state if needed, or just rely on logic
    // For now, keep simple logic equivalent to schema
  ...
  >({
    ...
    onError: (error) => {
      apiError.value = getErrorMessage(error, 'Erro ao finalizar OS');
    },
  });
