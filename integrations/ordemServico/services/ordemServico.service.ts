...
import type { PaginatedResponse, ApiError } from '@/shared/types/axios.types';
import type {
  OrdemServicoCreate,
  OrdemServicoRead,
...
import { getErrorMessage, isNetworkError } from '@/shared/utils/error.utils';
...
function handleServiceError(error: unknown, operation: string): never {
  ...
  const isNetwork = axiosError.response === undefined && isNetworkError(axiosError);
  const message = getErrorMessage(error, `Erro ao ${operation}`);

  throw new OSServiceError(message, operation, axiosError, isNetwork);
}
